...
def _ssl_context():
    ...
    Uses a bundled CA certificate file (cacert.pem) shipped with the plugin
    so HTTPS works regardless of the user's Python/OS certificate config.
    """
    bundle = os.path.join(os.path.dirname(__file__), "cacert.pem")
