...
- `id` values use **kebab-case** matching the blueprint template hierarchy.
...
Blueprint path keys use `/` separators with kebab-case ids: `executive-summary/objective`.
...
| `notes` | string[] | Section notes from `blueprint.section_notes`. |
| `footnotes` | string[] | Section footnotes from `blueprint.footnotes`. |
...
#### 1. Transactions overview (`preamble_transactions_overview`)
...
Source: `local_files[].covered_transactions` mapped to `transactions[]` in data.json.
...
Array of note groups for the side panel:
...
[
  {
    "scope": "group",
    "title": "Acme Group",
    "items": ["Key deadline: March 2025", "Check Dutch ruling status"]
  },
  {
    "scope": "entity",
    "title": "Acme NL B.V.",
    "items": ["New transaction added Q3"]
  },
  {
    "scope": "transaction",
    "title": "Licensing Fee",
    "items": ["Benchmark pending"]
  }
]
```

Source: `notes` arrays on group, entity, and transaction objects in data.json. Max 2 items per group.

## blueprints

Array of available blueprints for the blueprint picker modal:
...
[
  {
    ...
    "is_active": true,
    "badge": "builtin",
    "preview_chapters": ["Executive Summary", "Business Description"]
  }
]
```

Source: all `.json` files in the blueprints directory. `is_active` is true for the current blueprint. `badge` is `"builtin"` for standard templates, `"custom"` for firm/entity blueprints.

## jurisdiction_svg

HTML string containing an SVG map highlighting the entity's country. Empty string if no map data available. Rendered from `references/jurisdiction-maps.json`.
